...
router.put('/coursemaster/query', function(req, res){
    ...
    pg.connect(conString, function(err, client, done) {
        if(data.credits == -1){
          console.log("inside ilike function");
             query = client.query("SELECT * FROM coursemaster WHERE ((crid ILIKE $1) AND (dpid ILIKE $2) AND (coursename ILIKE $3))", [data.crid+ '%', data.dpid+ '%', data.coursename+ '%']);
        console.log("ilike performed");
        }
        else
        {
           query = client.query("SELECT * FROM coursemaster WHERE ((crid ILIKE $1) AND (dpid ILIKE $2) AND (coursename ILIKE $3) AND (credits = ($4)))", [data.crid+ '%', data.dpid+ '%', data.coursename+ '%', data.credits]);
        }
        query.on('row', function(row) {
          console.log(row);
            results.push(row);
...
router.delete('/faculty/:id', function(req, res) {

    var results = [];
    var id = req.params.id;
    pg.connect(conString, function(err, client, done) {
        client.query("DELETE FROM faculty WHERE fcid=($1)", [id]);
        console.log("Hey the record is deleted");
            // Handle Errors
            if(err) {
              console.log(err);
